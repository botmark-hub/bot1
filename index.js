...
async function searchSheet(keyword, sheetName) {
  ...
  for (const name of sheetNamesToSearch) {
    ...
    const usedHeaders = headers.filter((h) =>
      filtered.some((row) => row[h] !== '')
    );
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    const botDisplayName = botInfo.data.displayName
      .toLowerCase()
      .replace(/\s+/g, '');
    const mentionPattern = new RegExp(`@?${botDisplayName}`, 'gi');
    const cleanedMessage = textRaw
      .toLowerCase()
    ...
    if (cleanedMessage === 'ช่วยเหลือ') {
      ...
      if (!keyword && !sheetName) {
        await sendLongMessage(
          roomId,
          '⚠️ ต้องระบุคำค้นหาหรือชื่อแผ่นงาน'
        );
      } else {
...
// ✅ สำหรับ Render ให้ log port ธรรมดา
app.listen(PORT, () => {
